...
describe('i18n message key parity', () => {
  it('fr.json, en.json, zh.json, and es.json have exactly the same key structure', () => {
    ...
    const onlyInFr = frKeys.filter(
      (k) => !enKeys.includes(k) || !zhKeys.includes(k) || !esKeys.includes(k),
    );
    const onlyInEn = enKeys.filter(
      (k) => !frKeys.includes(k) || !zhKeys.includes(k) || !esKeys.includes(k),
    );
    const onlyInZh = zhKeys.filter(
      (k) => !frKeys.includes(k) || !enKeys.includes(k) || !esKeys.includes(k),
    );
    const onlyInEs = esKeys.filter(
      (k) => !frKeys.includes(k) || !enKeys.includes(k) || !zhKeys.includes(k),
    );
