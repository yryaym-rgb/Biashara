...
import type { MineralSegment } from '@/lib/admin/reports.logic';
...
export interface AdminMineralDistributionChartProps {
  segments: MineralSegment[];
}
...
function MixTooltip({ active, payload, ordersLabel }: MixTooltipProps) {
  ...
  return (
    <div className="rounded-card border border-border bg-bg px-3 py-2 card-shadow">
      ...
      <p className="tabular-nums text-[13px] text-muted">
        {point.payload.count} {ordersLabel}
      </p>
    </div>
  );
}
...
export function AdminMineralDistributionChart({ segments }: AdminMineralDistributionChartProps) {
  const t = useTranslations('admin.reports.charts.mineralDistribution');
            ...
            >
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip content={<MixTooltip ordersLabel={t('orders')} />} />
                </PieChart>
              </ResponsiveContainer>
            </div>
